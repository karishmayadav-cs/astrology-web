...
exports.SwissEph = exports.Constants = void 0;
exports.load = load;
...
Object.defineProperty(exports, "Constants", { enumerable: true, get: function () { return api_js_1.Constants; } });
/**
...
class SwissEph {
    constructor(module) {
        ...
        const exportsRecord = this.exports;
        ...
        this.heap = new heap_js_1.WasmHeap(this.instance.exports.memory, this.exports);
    }
    /**
...
async function load(options = {}) {
    let wasmSource;
    let ephePath;
    if (options instanceof Uint8Array || typeof options === "string" ||
    ...
    if (wasmSource instanceof Uint8Array) {
        ...
        const url = wasmSource ||
            new URL("../lib/wasi/swiss_eph.wasm", globalThis[Symbol.for("import-meta-ponyfill-commonjs")](require, module).url);
        if (typeof dntShim.Deno !== "undefined") {
            bytes = await dntShim.Deno.readFile(url);
        }
        else {
    ...
    if (ephePath && typeof dntShim.Deno !== "undefined") {
        // Recursively load .se1 and .sweph files from the path
        for await (const entry of dntShim.Deno.readDir(ephePath)) {
            if (entry.isFile &&
                (entry.name.endsWith(".se1") || entry.name.endsWith(".sweph"))) {
                const content = await dntShim.Deno.readFile(`${ephePath}/${entry.name}`);
